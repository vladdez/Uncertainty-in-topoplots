...
The analysis contains 213 evaluative meaning units from 49 participants. `ID` links every unit to its original A305 response. Participant wording is retained as closely as possible; the theme and intensity are researcher-assigned codes. The complete coded table is shown below.
...
a305_content_analysis |>
  ...
  knitr::kable(
    ...
    caption = "Coded evaluative meaning units from A305 responses.",
    align = c("c", "l", "l", "l", "c"),
    table.attr = kable_table_attributes
  )
```
...
a305_intensity_summary |>
  mutate(
    ...
    median_intensity = sprintf("%.2f", median_intensity),
    mean_intensity = sprintf("%.2f", mean_intensity)
  ) |>
  select(
    theme,
    visualization,
  ...
  knitr::kable(
    ...
    caption = "Participant-based evaluation-intensity summary.",
    align = c("l", "l", "r", "r", "r"),
    table.attr = kable_table_attributes
  )
```

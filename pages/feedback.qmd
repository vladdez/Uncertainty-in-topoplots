...
**Meaning units extracted:** `r nrow(a305_content_analysis)` 
...
visualization_order_with_general_bivariates <- append(
  visualization_order,
  "General bivariates",
  after = 4
)

summarise_a305_participant_theme <- function(visualization_levels) {
  a305_content_analysis |>
    transmute(
      participant = ID,
      visualization = factor(Visualization, levels = visualization_levels),
      theme = Theme,
      intensity = as.numeric(
        gsub("+", "", `Evaluation intensity`, fixed = TRUE)
      )
    ) |>
...
a305_participant_theme <- summarise_a305_participant_theme(
  visualization_order
)

a305_participant_theme_with_general_bivariates <-
  summarise_a305_participant_theme(
    visualization_order_with_general_bivariates
  )
...
visualization_axis_labels <- c(
  ...
  `General bivariates` = "General<br>bivariates",
  `Confidence interval` = "Confidence<br>interval",
  `Uncertainty markers` = "Uncertainty<br>markers",
  Animation = "Animation"
)

make_a305_intensity_figure <- function(plot_data, angle_x_labels = FALSE) {
  theme_panel_counts <- plot_data |>
...
a305_intensity_by_theme_figure <- make_a305_intensity_figure(
  a305_participant_theme
)
...
## Figure including general bivariate feedback — Expand

This additional version includes comments that evaluate bivariate plots in
general without identifying either the corner or range variant.

```{r a305-intensity-theme-figure-general-bivariates}
#| echo: false
#| fig-width: 15
#| fig-height: 19
#| out-width: 100%
#| fig-alt: "Faceted boxplots of participant-level median evaluation intensity by visualization type and qualitative theme, including general bivariate feedback, with non-overlapping individual participant points."

a305_intensity_by_theme_with_general_bivariates_figure <-
  make_a305_intensity_figure(
    a305_participant_theme_with_general_bivariates,
    angle_x_labels = TRUE
  )
...
ggsave(
  project_path(
    "figures",
    "a305_intensity_by_theme_with_general_bivariates.png"
  ),
  a305_intensity_by_theme_with_general_bivariates_figure,
  width = 15,
  height = 19,
